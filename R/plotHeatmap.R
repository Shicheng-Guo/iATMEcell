...
##' @usage plotHeatmap(Regress.list,ExpData,p.cutoff=0.01,cluster.rows=F,
...
##'       p.cutoff=0.01,method = 'univ')
##' plotHeatmap(Regress.list=R.result,ExpData=GEP,p.cutoff=0.01)



plotHeatmap<-function(Regress.list,ExpData,p.cutoff=0.01,cluster.rows=F,
         cluster.cols=F,bk=c(-2.4,2.3),show.rownames=T,show.colnames=F,
         ann_colors=c("#FFAA2C","#2CBADA"),col=c("#2A95FF","#FF1C1C")){
  GEP<-ExpData
  ...
  GEP<-GEP[rownames(GEP)%in%sig.genes,]
